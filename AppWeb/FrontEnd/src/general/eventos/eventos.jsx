...
import EventosDetalhes from "./eventos-detalhes";
...
export default function Eventos(props) {
  ...
  const service = props.config.service + "evento"
  ...
  const [filterStatus, setFilterStatus] = React.useState("00");
  ...
  const editHandleClick = (event) => {    
    ...
    //   ...props.inputValues,
    //   status: "00",
    //   statusMotivo: "Reprocessar evento",
    // });
  ...
  async function list(page, rowsPerPage) {
    ...
    await ApiUtilities.fetch(service, {page: page , limit : rowsPerPage , filter: filter, filterStatus: filterStatus, order:"USUAN_DATA DESC"}, "GET")
      .then((res) => {
        res.json().then((res) => {
        ...
        res.json().then((res) => {
          ...
          if (res.Code === 200) {
            ...
            setInputValues({
              ...
              evento: model.evento || "",
              status: model.status || "",
              statusMotivo: model.statusMotivo || "",

              // status:"00",
              // statusMotivo: "Reprocessar evento",
              retornoEvento: model.retornoEvento || ""
            });
  ...
  // Imprime PDF
  // 
  async function reprocessar(event) {

    setLoading(true);

    let id = anchorEl.parentElement.parentElement.cells[0].innerText;

    ApiUtilities.fetch(service + "/reprocessar", { id: id }, "GET")
    
    // await ApiUtilities.fetch(service + "/reprocessar", inputValues, "POST")
      .then((res) => {
        res.json().then((res) => {
    ...
    <div>
      <Paper className={classes.paper}>
        ...
        <EventosLista
          ...
          filterChangeStatus={(event) => { setFilterStatus(event.target.value) }}
        />
